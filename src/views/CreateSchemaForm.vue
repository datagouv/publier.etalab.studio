<template>
  <div class="fr-container fr-pb-6w fr-pt-2w">
      ...
      <b-form @submit="onSubmit1" @reset="onReset" v-if="show">
        ...
        >
          ...
          <template slot="description">
            <p>
              Nom court du schéma, sans accent ni espace (séparer les mots d'un
              tiret).
            </p>
          </template>
        </b-form-group>
        ...
        >
          ...
          <template slot="description">
            <p>
              Titre du schéma. Qu'est-ce que ce schéma est censé représenté ?
            </p>
          </template>
        </b-form-group>
        ...
        <div style="text-align: center">
          <b-button type="submit" class="fr-btn">Suivant</b-button>
        </div>
      </b-form>
      <br />
    </div>
    <div v-if="showLevel2">
      ...
      <br />
      <b-button
        @click="showContributorForm = true"
        class="fr-btn"
        v-if="!showContributorForm"
        >Ajouter un nouveau contributeur</b-button
      >
      ...
      <b-form @submit="onSubmit2" v-if="showContributorForm">
        <b-form-group
          id="input-group-contributorTitle"
          label="Contributeur : "
          label-for="input-contributorTitle"
        >
          <b-form-input
        ...
        <b-form-group
          id="input-group-contributorEmail"
          label="Email : "
          label-for="input-contributorEmail"
        >
          <b-form-input
        ...
        <b-form-group
          id="input-group-contributorOrganisation"
          label="Organisation : "
          label-for="input-contributorOrganisation"
        >
          <b-form-input
        ...
        <br />
        <div style="text-align: center">
          <b-button type="submit" class="fr-btn"
            >Valider le contributeur</b-button
          >
        </div>
      </b-form>
      <br />
      <div style="text-align: center" v-if="!showContributorForm">
        <b-button @click="previous1()" class="fr-btn">Précédent</b-button
        >&nbsp;&nbsp;
        <b-button @click="next1()" class="fr-btn">Suivant</b-button>
      </div>
    </div>
    ...
    <div v-if="showLevel3">
      ...
      <b-button
        ...
        class="fr-btn"
        v-if="!showFieldForm"
        >Ajouter un nouveau champs</b-button
      >
      ...
      <b-form @submit="onSubmit3" v-if="showFieldForm">
        <b-form-group
          id="input-group-field-type"
          label="Type : "
          label-for="input-group-field-type"
        >
          <b-form-select
            v-model="field.type"
          ...
          >
            <b-form-select-option disabled value=""
              >Sélectionner un type...</b-form-select-option
            >
            <b-form-select-option
              v-bind:key="dataType.name"
              v-bind:value="dataType.name"
              v-for="dataType in dataTypes"
              >{{ dataType.nameFR }}</b-form-select-option
            >
          </b-form-select>
          <br />
        </b-form-group>
        <div v-if="field.formats && field.formats.length > 0">
          <b-form-group
            id="input-group-field-format"
            label="Format : "
            label-for="input-group-field-format"
          >
            <b-form-select v-model="field.format" class="fr-select">
              <b-form-select-option disabled value=""
                >Sélectionner un format...</b-form-select-option
              >
              <b-form-select-option
                v-bind:key="format.name"
                v-bind:value="format.name"
                v-for="format in field.formats"
                >{{ format.nameFR }}</b-form-select-option
              >
            </b-form-select>
            <br />
          </b-form-group>
          <div v-if="field.format == 'pattern'">
            <b-form-group
              id="input-group-formatPattern"
              label="Motif : "
              label-for="input-formatPattern"
            >
              <b-form-input
        ...
        <div>
          <div>
            ...
            >
              ...
              <template slot="description">
                <p>
                  Le nom du champs. Dans la mesure du possible, choisir un nom
                  sans majuscule, sans espace, sans accent et séparé par des
                  underscores.
                </p>
              </template>
            </b-form-group>
            <b-form-group
              id="input-group-field-description"
            ...
            <b-form-group
              id="input-group-field-example"
              label="Exemple : "
              label-for="input-field-example"
            >
              <b-form-input
        ...
        <div>
          ...
          <div v-if="showConstraintForm">
            <b-form-group
              id="input-group-constraint-name"
              label="Type : "
              label-for="input-constraint-name"
            >
              <b-form-select
              ...
              >
                <b-form-select-option disabled value=""
                  >Sélectionner un type de contrainte...</b-form-select-option
                >
                <b-form-select-option
                  v-bind:key="constraint.name"
                  v-bind:value="constraint.name"
                  v-for="constraint in constraints"
                  >{{ constraint.nameFR }}</b-form-select-option
                >
              </b-form-select>
              <br />
            </b-form-group>
            <b-form-group
              v-if="
                currentConstraint &&
                (currentConstraint.dataType == 'string' ||
                  currentConstraint.dataType == 'integer')
              "
              id="input-group-constraint-value-string-integer"
              label="Valeur : "
              label-for="input-constraint-value-string-integer"
            >
              <b-form-input
            ...
            <b-form-group
              v-if="
                currentConstraint && currentConstraint.dataType == 'boolean'
              "
              id="input-group-constraint-value-boolean"
              label="Type : "
              label-for="input-group-constraint-value-boolean"
            >
              <b-form-select
                v-model="currentConstraint.value"
                class="fr-select"
            ...
            <b-form-group
              v-if="currentConstraint && currentConstraint.dataType == 'array'"
              id="input-group-constraint-value-array"
              label="Liste de valeurs : "
              label-for="input-group-constraint-value-array"
            >
              <b-form-tags
            ...
            <b-button @click="submitConstraintForm()" class="fr-btn-light"
              >Valider la contrainte</b-button
            >
          </div>
        </div>
        <div>
        ...
        <div style="text-align: center">
          <b-button type="submit" class="fr-btn">Valider le champs</b-button>
        </div>
      </b-form>
      <div style="text-align: center" v-if="!showFieldForm">
        <b-button @click="previous2()" class="fr-btn">Précédent</b-button
        >&nbsp;&nbsp;
        <b-button @click="next2()" class="fr-btn">Suivant</b-button>
      </div>
    </div>
    ...
    <div v-if="showLevel4">
      ...
      <div style="text-align: center">
        <b-button @click="validateSchema()" class="fr-btn"
          >Valider le schéma</b-button
        >
      </div>
    </div>
    ...
    <div v-if="showLevel5">
      <div style="text-align: center">
        <br />
        <br />
        <b-button :href="this.linkSchemaCreated" class="fr-btn"
          >Télécharger votre schéma au format json</b-button
        >
        <br />
        <br />
        <b-button @click="goto()" class="fr-btn"
          >Saisir ou valider des données basées sur ce schéma.</b-button
        >
        <br />
        <br />
        <button
        ...
        >
          ...
          <div style="flex: 1 1 auto">
            <span
              v-if="visibleCopy"
              style="background-color: #dfdff1; color: #060191; padding: 5px"
              >{{ copyMessage }}</span
            >
          </div>
        </button>
...
export default {
  ...
  methods: {
    ...
    submitConstraintForm() {
      ...
      if (
        this.currentConstraint.values &&
        this.currentConstraint.values.length > 0
      ) {
    ...
    validateSchema() {
      ...
      instance
        .post(
          `${process.env.VUE_APP_DEMO_API_URL}/api/1/datasets/60b4b9b37b3c3a33db85ba89/upload/`,
          formData,
          { headers: headers }
        )
        .then((response) => {
          ...
          this.pageTitle =
            "Félicitations, votre schéma est désormais disponible";
        })
    ...
    goto() {
      this.$router.push({
        ...
        query: { schema_url: this.linkSchemaCreated },
      });
    },
    onCopy: function (e) {
